	...
	<view>
		<view id="con">
			...
			<view id="showUserList">
				...
				<view id="showUserList_info" class="blockcenter" v-for="(item, index) in userList" :key="item.phone">
						...
						<swiper class="showUser_swiper">
							...
							<swiper-item>
								<view class="userList_index">
									<view>
										<view>
											<input type="text" placeholder="请输入新的密码" :value="item.passwd" ref="changePasswd" confirm-type="done" />
										</view>
	...
	export default {
		...
		methods: {
			...
			changeUserInfo(index) {
				//this.$refs亲测无效
				// this.$nextTick(() => {
				// 	const itemRefs = this.$refs.changePasswd
				// 	let nowPasswd = itemRefs[index].value
				// 	console.log('nowPasswd:')
				// 	console.log(nowPasswd)
				// })
				uni.showToast({
					title: "该功能尚未实现,按下按钮后不会做任何操作",
					icon: 'none'
				})
			},
			choUser(user) {
